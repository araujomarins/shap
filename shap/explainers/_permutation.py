...
class Permutation(Explainer):
    ...
    def explain_row(
        ...
        *row_args,
        ...
        if len(inds) > 0:
            for _ in range(npermutations):
                ...
                for ind in inds:  # forward
                    ...
                    if error_bounds:
                        row_values_history[history_pos][ind] = (
        ...
        return {
            ...
            "error_std": None
            if row_values_history is None
            else row_values_history.std(0),
    ...
    def shap_values(
        ...
        """
        warnings.warn(
            "shap_values() is deprecated; use __call__().", DeprecationWarning
        )

        explanation = self(
            X, max_evals=npermutations * X.shape[1], main_effects=main_effects
        )
